...
SELECT
    ...
    AVG(CASE WHEN itemid = 220045 THEN average_valuenum ELSE NULL END) AS pulse
    -- MAX(CASE WHEN itemid = 220045 AND average_valuenum > 90 THEN 1 ELSE 0 END) AS Tachy
FROM hourly_averages
GROUP BY hadm_id, subject_id, hour_interval
ORDER BY hadm_id, subject_id, hour_interval
) TO :'DATA_DIR'/Tachy.csv WITH CSV HEADER;
