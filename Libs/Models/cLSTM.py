...
class cLSTM(nn.Module):
    ...
    def attention(self, *, temperature: float = 1.0) -> torch.Tensor:  # noqa: N802
        """Row-normalised attention matrix from LSTM input weights."""
        w = [torch.norm(net.lstm.weight_ih_l0, dim=0) for net in self.networks]
...
def train_model_adam(clstm, X, context, lr, max_iter, lam=0, lam_ridge=0,
    ...
    for it in range(max_iter):
        ...
        pred = [clstm.networks[i](X_batch)[0] for i in range(p)]
